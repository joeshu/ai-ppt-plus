...
|---|---|---|
| `ai-ppt-plus` | intake, source authority, narrative, PPT thought-table governance, approved outline, route, design authority, manifest reconciliation, QA aggregation, reports, human closeout and release gates | worker-internal generation/decomposition algorithms |
| `ai-ppt-visual-gen` | approved-table consumption, A1–A5 planning and prompts, premium-commercial visual quality contract, same-model/context continuity, raster generation, page-local retry, generated-source retention and deck-strip review | narrative approval/formal-text authority, editable reconstruction, release or human sign-off |
...
2. A request only for image-format slides or a visual intermediate may invoke
   `ai-ppt-visual-gen` directly. If it receives only a topic or an unapproved
   draft, it first produces the four-column PPT thought table and stops for
   review. Under orchestration, it consumes the approved table/design revisions
   and returns evidence without replacing them.
...
- `ai-ppt-plus/narrative-gate/v1` for the approved PPT thought-table handoff;
