...
metadata:
  package_revision: 2026.09.23.06
---
...
Reviewed visual precedents are optional runtime inputs, not required package
files. If a project supplies a case directory, read its local `README.md` and
`case.json` before use and record the case ID in the active plan. Use
...
copy contract remain authoritative; never reuse a case's text, numbers,
claims, logos, citations or distinctive brand assets. A missing optional case
must not block package validation or be silently reconstructed from memory.
