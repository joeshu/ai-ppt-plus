...
def main() -> int:
    ...
    if args.object_manifest:
        ...
        for slide in data.get("slides", []):
            for obj in slide.get("objects", []):
                expected[obj.get("object_id")] = obj
    ...
    for si, slide in enumerate(prs.slides, 1):
        ...
        if args.require_independent_panels:
            panel_ids = [oid for oid, obj in expected.items() if obj.get("slide_no", si) == si and obj.get("role") in {"semantic-panel", "panel", "frame-panel"}]
    ...
    if whole_slide_images and expected:
