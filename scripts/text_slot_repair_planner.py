...
REPAIR_ORDER = [
    "slot_bbox",
    "text_margins",
    "reserved_icon_divider_space",
    "component_layout",
    "reference_line_topology",
    "line_spacing",
    "font_size_last",
]
...
def _protected(owner: dict, objects: dict[str, dict]) -> tuple[list[str], list[dict]]:
    ...
    rows = [objects[value] for value in ids if value in objects]
    return ids, rows
...
def _safe_bbox_patch(owner: dict, protected_rows: list[dict], deficit_px: list[float], source_w: float, source_h: float) -> dict | None:
    ...
    if box is None or source_w <= 0 or source_h <= 0:
        return None
    dw = deficit_px[0] / source_w
    dh = deficit_px[1] / source_h
    if dw <= 0 and dh <= 0:
        return None
    ...
    for candidate in _bbox_candidates(box, dw, dh):
        if not any(_overlap(candidate["bbox"], blocker) for blocker in blockers):
            return {
                "bbox": candidate["bbox"],
                "growth_norm": [round(dw, 8), round(dh, 8)],
                "growth_px": [round(deficit_px[0], 3), round(deficit_px[1], 3)],
                "anchor_mode": candidate["mode"],
            }
    return None


def _margin_candidate(contract: dict, deficit_px: list[float]) -> dict | None:
    margins = contract.get("margins_px")
    minimum = contract.get("min_margins_px")
    if not (isinstance(margins, (list, tuple)) and len(margins) == 4 and isinstance(minimum, (list, tuple)) and len(minimum) == 4):
        return None
    current = [max(0.0, _num(v)) for v in margins]
    floor = [max(0.0, _num(v)) for v in minimum]
    ...
    if deficit_px[0] > available_w + 1e-6 or deficit_px[1] > available_h + 1e-6:
        return None
    need_w, need_h = deficit_px
    next_margins = list(current)
    for index in (0, 2):
...
def _line_spacing_candidate(contract: dict) -> dict | None:
    current = contract.get("line_spacing")
    minimum = contract.get("min_line_spacing")
...
def _font_candidate(slot: dict, contract: dict) -> dict | None:
    target = _num(slot.get("target_pt"), 0)
    recommended = _num(slot.get("recommended_pt"), 0)
    minimum = _num(contract.get("min_font_size_pt"), 0)
    if target <= 0 or recommended <= 0 or recommended >= target:
        return None
    if minimum > 0 and recommended < minimum:
        return None
...
def _responsible_parameter(step: str) -> str:
    return {
        ...
        "reserved_icon_divider_space": "bbox",
        "component_layout": "bbox",
        "reference_line_topology": "bbox",
...
def _action(slot: dict, owner_id: str, owner: dict, objects: dict[str, dict], source_w: float, source_h: float) -> dict:
    ...
    if topology_defect:
        topology_patch = {
            "target_lines": int(slot.get("target_lines") or 0),
            "observed_lines": int(slot.get("line_count") or 0),
            "line_count_delta": int(slot.get("line_count") or 0) - int(slot.get("target_lines") or 0),
            "mode": "preserve_text_content_reflow_only",
        }
    ...
    return {
        ...
        "producer_kind": slot.get("producer_kind"),
        "classification": "text_slot_layout",
        "repair_required": True,
        "defect": {
            "geometry_defect": geometry_defect,
            "topology_defect": topology_defect,
            "box_deficit_px": [round(v, 3) for v in deficit],
            "target_lines": slot.get("target_lines"),
            "observed_lines": slot.get("line_count"),
            "reference_scale": slot.get("reference_scale"),
        },
        "repair_order": REPAIR_ORDER,
        "candidates": candidates,
        "selected_step": selected["step"] if selected else "manual_layout_review",
        "responsible_parameters": [_responsible_parameter(selected["step"])] if selected else [],
        "proposed_patch": proposed_patch,
        "protected_neighbors": protected_ids,
        "forbidden_compensation": protected_ids,
        "font_shrink_last": True,
        "requires_render_replay": True,
        "repair": {"classification": "text_slot_layout", "regenerate_asset": False},
    }
...
def build_plan(text_fit_report: dict, authoring_plan: dict) -> dict:
    ...
    source_w = _num(source.get("width_px"), 0)
    source_h = _num(source.get("height_px"), 0)
    ...
    return {
        "schema": SCHEMA,
        "valid": not issues,
        "source": {"text_fit_schema": text_fit_report.get("schema"), "authoring_plan_schema": authoring_plan.get("schema")},
        "policy": {"repair_order": REPAIR_ORDER, "font_shrink_last": True, "move_unrelated_objects": False, "require_render_replay": True},
        "summary": {
            "action_count": len(actions),
            "unresolved_count": len(unresolved),
            "bbox_first_count": sum(row["selected_step"] == "slot_bbox" for row in actions),
            "font_last_count": sum(row["selected_step"] == "font_size_last" for row in actions),
        },
        "actions": actions,
        "issues": issues,
    }
