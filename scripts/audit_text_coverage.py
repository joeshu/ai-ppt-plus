...
def main():
    p=argparse.ArgumentParser(); p.add_argument("authoring_plan",type=Path); p.add_argument("text_ledger",type=Path); p.add_argument("--output",type=Path,required=True); a=p.parse_args(); result=audit(json.loads(a.authoring_plan.read_text(encoding="utf-8")),json.loads(a.text_ledger.read_text(encoding="utf-8"))); a.output.parent.mkdir(parents=True,exist_ok=True); a.output.write_text(json.dumps(result,ensure_ascii=False,indent=2)+"\n",encoding="utf-8"); print(json.dumps({"valid":result["valid"],"coverage":result["coverage"],"output":str(a.output)})); return 0 if result["valid"] else 2
