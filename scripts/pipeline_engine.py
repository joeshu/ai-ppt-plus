...
class PipelineExecutor:
    ...
    def __init__(
        ...
        *,
        ...
        # Validators are a mixed CPU/I/O workload.  Leave one CPU available
        # and cap concurrency to prevent simultaneous render/image processes
        # from multiplying peak memory.
    ...
    def _write_checkpoint(self, results: dict[str, dict[str, Any]], pending: set[str], *, status: str = "running") -> None:
        ...
        for name, result in results.items():
            ...
            if task:
                for output in task.outputs:
                    record = self._output_record(output)
                    if record:
                        outputs.append(record)
            checkpoint["completed"].append({
                "name": name, "ok": result.get("ok"), "cache_hit": result.get("cache_hit"),
                "duration_ms": result.get("duration_ms", 0), "cache_key": result.get("cache_key"),
    ...
    def _resume_results(self, by_name: dict[str, PipelineTask]) -> dict[str, dict[str, Any]]:
        if not self.resume:
            return {}
        path = self.run_dir / "pipeline-checkpoint.json"
        try:
            checkpoint = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            return {}
        if checkpoint.get("schema") != "ai-ppt-plus/pipeline-checkpoint/v2" or checkpoint.get("engine_version") != ENGINE_VERSION:
            return {}
        if checkpoint.get("local_code_fingerprint") != self.code_fingerprint or checkpoint.get("runtime_fingerprint") != self.runtime_fingerprint:
            return {}
        restored = {}
        for record in checkpoint.get("completed", []):
            ...
            valid = len(expected) == len(task.outputs)
            for output in task.outputs:
                current = self._output_record(output)
                if not current or expected.get(current["relative"], {}).get("sha256") != current.get("sha256"):
                    valid = False
                    break
            if valid:
                result = dict(record.get("result") or {})
                result.update({"name": name, "resumed": True, "cache_hit": False, "duration_ms": 0.0})
    ...
    def run(self) -> list[dict[str, Any]]:
        ...
        capacity_ms = self.last_wall_duration_ms * self.max_workers
        self.last_idle_wait_ms = round(max(0.0, capacity_ms - task_duration_sum), 3)
        self.last_output_bytes = sum(_tree_size(path) for task in self.tasks for path in task.outputs if path.exists())
        self.last_temporary_bytes = sum(
            child.stat().st_size for child in self.run_dir.rglob("*")
            if child.is_file() and child.name.startswith(".") and ".tmp" in child.name
        )
