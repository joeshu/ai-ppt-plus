...
def main() -> int:
    ...
    if canvas_report:
        quality_evidence["canvas_evidence"] = {
            "valid": canvas_report.get("valid"),
            "status": canvas_report.get("status"),
            "exact_canvas": canvas_report.get("exact_canvas"),
            "expected_pages": canvas_report.get("expected_pages"),
            "mismatches": canvas_report.get("mismatches", []),
            "issues": canvas_report.get("issues", []),
            "warnings": canvas_report.get("warnings", []),
        }
    ...
    if host_report:
        quality_evidence["host_validation"] = {
            "valid": host_report.get("valid"),
            "status": host_report.get("status"),
            "host": host_report.get("host"),
            "reviewer": host_report.get("reviewer"),
            "confirmed_at": host_report.get("confirmed_at"),
            "checked_slides": host_report.get("checked_slides"),
            "checks": host_report.get("checks", {}),
            "issues": host_report.get("issues", []),
        }
    ...
    if args.require_visual_threshold:
        ...
        observed = None
        if visual and isinstance(visual.get("metrics"), dict):
            observed = visual["metrics"].get("blurred_layout_ssim")
        if observed is None:
