...
def _slots(deck:dict):
    for slide_no,slide in enumerate(deck.get("slides") or [],1):
        ...
        for chart_index,chart in enumerate(slide.get("charts") or []):
            ...
            if title:
                spec={"text":str(title),"font_size_pt":chart.get("title_font_size_pt",16),"font":chart.get("font") or deck.get("font_family")}; chart_w,chart_h=_box_px(deck,chart)
...
def audit_layout(layout:Path,*,font_file:str|None=None)->dict:
    deck=json.loads(layout.read_text(encoding="utf-8")); slots=[]; coverage={"text":0,"shape_text":0,"table_cell":0,"chart_title":0}
...
def main()->int:
    parser=argparse.ArgumentParser(description=__doc__); parser.add_argument("layout",type=Path); parser.add_argument("--font-file"); parser.add_argument("--report",type=Path,required=True); parser.add_argument("--fail-on-target-not-fit",action="store_true"); parser.add_argument("--require-kind",action="append",choices=["text","shape_text","table_cell","chart_title"],default=[]); args=parser.parse_args()
