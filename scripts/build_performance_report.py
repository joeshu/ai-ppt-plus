...
def build(pipeline_result: Path, output: Path, *, issue_log: Path | None = None, repair_round: int | None = None) -> dict[str, Any]:
    ...
    report = {
        ...
        "execution": {
            ...
            "scheduler_idle_capacity_ms": float(execution.get("scheduler_idle_capacity_ms", 0) or 0),
            "resume_hits": _integer(execution.get("resume_hits")),
            "retry_count": retries,
            "output_bytes": _integer(execution.get("output_bytes")),
            "temporary_bytes": _integer(execution.get("temporary_bytes")),
            "resource_usage": execution.get("resource_usage") if isinstance(execution.get("resource_usage"), dict) else {
                "peak_rss_bytes": None, "read_bytes": None, "write_bytes": None, "available": False,
            },
            "external_calls": execution.get("external_calls") if isinstance(execution.get("external_calls"), dict) else {},
