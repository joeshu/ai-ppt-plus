...
def main() -> int:
    ...
    if args.require_source_visual_assets:
        ...
        if not source_visual_inventory_path.is_file():
            ...
            source_visual_args = [str(SCRIPT_DIR / "validate_source_visual_assets.py"), "--inventory", str(source_visual_inventory_path), "--report", str(run_dir / "source-visual-assets-validation.json")]
            if (project / "imagegen-assets-manifest.json").is_file():
    ...
    def build_pipeline_result(current_steps, evidence, degradations, release_report=None, bundle_report=None, signoff_report=None):
        ...
        return {
            ...
            "execution": {
                ...
                "external_calls": {
                    "imagegen": sum(1 for step in current_steps if "imagegen" in step.get("name", "")),
                    "ocr": sum(1 for step in current_steps if "ocr" in step.get("name", "")),
                    "pptx_render": sum(1 for step in current_steps if step.get("name") == "render"),
                    "visual_compare": sum(1 for step in current_steps if step.get("name") in {"visual-comparison", "visual-compare-qa", "dual-comparison"}),
                },
