...
6. `skill-routing/v1` and `engine-route-validation/v1` bind editable
   reconstruction/native authoring to `ai-ppt-editable` and formal PPTX
   authoring to `@oai/artifact-tool`. ImageGen is limited to independent visual
   assets. Missing capability fails closed; external skill/backend and
   source-crop fallbacks are forbidden.
