...
For an operator who has just completed human review, use
`scripts/ingest_approved_case.py`. It calls `training_export.py approve-case`
with the required explicit confirmation and then invokes the same training
cycle driver, so the manual approval boundary and the unattended export path
cannot drift apart.
