...
- All icons, decorations, logos and artistic typography in a reference reconstruction must pass the B4 asset-provenance inventory and B5 cutout/split QA where applicable. Use the native ImageGen route for every brand-class visual and every missing, ambiguous or reconstructive visual. A source crop is reference evidence only; non-brand source reuse is allowed only after an explicit user-approved exact-pixel request or fallback decision. Every final asset remains independently movable and provenance-validated.
...
- Do not force a low-quality native PowerPoint redraw of complex illustration, photography, texture or specialized visual material. Brand visuals must use native ImageGen and remain blocked if generation fails; non-brand supplied assets may use an explicitly approved exact-pixel fallback. If a required asset is missing and no approved fallback exists, create a precisely sized and positioned placeholder and emit a material request.
