...
- All icons, decorations, logos and artistic typography must pass the B4 asset-provenance inventory and B5 cutout/split QA where applicable. Use native ImageGen for missing, ambiguous or reconstructive visuals. A separately supplied exact official/standalone brand file may be reused with hash/provenance evidence; a reference-slide crop is never that route. Non-brand source reuse still requires an explicit user-approved exact-pixel request or fallback decision. Every final asset remains independently movable and provenance-validated.
...
- Do not force a low-quality native PowerPoint redraw of complex illustration, photography, texture or specialized visual material. Brand visuals use native ImageGen unless an exact independent official/standalone file is supplied. Non-brand supplied assets may use an explicitly approved exact-pixel fallback. If a required asset is missing and no approved fallback exists, create a precisely sized and positioned placeholder and emit a material request.
