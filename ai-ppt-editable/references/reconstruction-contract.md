...
- All icons, decorations, logos and artistic typography must pass the B4 asset-provenance inventory and B5 cutout/split QA where applicable. Use native ImageGen for every brand visual's final asset, whether its reference is a screenshot or a separately supplied official file; source files and crops guide generation but are not inserted as final brand assets. Non-brand source reuse still requires an explicit user-approved exact-pixel request or fallback decision. Every final asset remains independently movable and provenance-validated.
...
- Do not force a low-quality native PowerPoint redraw of complex illustration, photography, texture or specialized visual material. Brand visuals always use native ImageGen for the final asset, including when an exact independent official/standalone file is supplied as a reference. Non-brand supplied assets may use an explicitly approved exact-pixel fallback. If a required asset is missing and no approved fallback exists, create a precisely sized and positioned placeholder and emit a material request.
...
- Do not generatively recreate charts, tables, names, numbers or unreadable formal text as if they were verified originals. Brand visuals are the explicit ImageGen asset class; do not convert their internal lettering into ordinary native text.
...
2. **Create an object decomposition plan:** map each visible element to one of `editable_text`, `native_shape`, `native_group`, `editable_vector`, `editable_chart`, `independent_image`, `extracted_icon`, `decorative_art`, `traceable_static_graphic`, or `documented_placeholder`; use `flattened_full_slide` and `unresolved` only to produce a blocking finding. For icons/decorations/artistic typography, first apply `references/icon-asset-protocol.md`, then assign the corresponding `L0`–`L5` level, provenance, required-for-delivery flag and human-review flag. See `references/editability-levels.md`.
...
Allowed without redesign: a semantically equivalent generic vector icon only after the B4/B5 provenance route; a metrically compatible font when the specified font is unavailable and substitution is disclosed; a traceable static chart image only when editable-chart support is unavailable and the user accepts the explicit `L3` degradation. Independent supplied photos/textures/icons are `L2` only when their provenance and replacement path are recorded. Missing complex assets are `L4` placeholders, not invented artwork.
