...
The PageGraph is the visual-geometry authority for reference reconstruction.
This adapter deterministically projects normalized graph bboxes onto matching
authoring objects before text-fit/typography repair.  It never invents objects
or edits content/style/assets; unmatched or low-confidence graph nodes remain
for the subsequent fail-closed geometry gate to report.
"""
...
def _write_bbox(spec:dict,box:list[float])->None:
    ...
    if isinstance(spec.get("bbox"),list):
        spec["bbox"]=[x,y,w,h]
    # Keep the common authoring aliases synchronized so downstream backends do
    # not accidentally continue using stale geometry from another field.
...
def apply_page_graph_geometry(deck:dict,page_graph:dict,*,min_confidence:float=0.5)->tuple[dict,dict]:
    ...
    for node in page_graph.get("nodes") or []:
        ...
        try:
            ...
            skipped.append({"object_id":oid,"reason":"invalid_geometry_or_confidence"})
            continue
        if confidence<min_confidence:
            skipped.append({"object_id":oid,"reason":"low_confidence","confidence":confidence})
            continue
        target=index.get(oid)
        if not target:
            missing.append({"object_id":oid,"node_type":node.get("type"),"confidence":confidence})
            continue
        box=_convert_bbox(result,values)
        before=[target["spec"].get(k) for k in ("x","y","w","h")]
        _write_bbox(target["spec"],box)
        target["spec"]["page_graph_geometry_locked"]=True
        target["spec"]["page_graph_confidence"]=confidence
    ...
    report={
        "schema":"ai-ppt-plus/page-geometry-calibration/v2",
        "valid":True,
        ...
        "missing":missing,
        "skipped":skipped,
        "policy":"pagegraph_bbox_is_authoritative_before_text_fit",
    }
...
def main()->int:
    ...
    p.add_argument("layout",type=Path);p.add_argument("page_graph",type=Path)
    p.add_argument("--output",type=Path,required=True);p.add_argument("--report",type=Path,required=True)
    p.add_argument("--min-confidence",type=float,default=0.5)
    a=p.parse_args()
    r=calibrate_layout(a.layout.resolve(),a.page_graph.resolve(),a.output.resolve(),a.report.resolve(),a.min_confidence)
    print(json.dumps({"schema":r["schema"],"valid":r["valid"],"applied_count":r["applied_count"],"missing_count":r["missing_count"]},ensure_ascii=False))
    return 0
