#!/usr/bin/env python3
"""Apply authoritative PageGraph geometry and text evidence before authoring."""
...
TEXT_EVIDENCE_FIELDS=("reference_line_count","reference_line_boxes","source_bbox","font_size_pt","font","bold","color","line_spacing","align","valign")
    ...
    for slide_no,slide in enumerate(deck.get("slides") or [],1):
        ...
        for collection in COLLECTIONS:
            for spec in slide.get(collection) or []:
                ...
                if oid and oid not in result: result[oid]={"slide":slide_no,"collection":collection,"spec":spec}
    return result
...
def _convert_bbox(deck:dict,bbox:list[float])->list[float]:
    ...
    if deck.get("units","fraction")=="px":
        ...
        if rw<=0 or rh<=0: raise ValueError("px layout requires positive reference dimensions")
...
def _write_bbox(spec:dict,box:list[float])->None:
    x,y,w,h=box
    if isinstance(spec.get("bbox"),list): spec["bbox"]=[x,y,w,h]
    for key,value in (("x",x),("y",y),("w",w),("h",h),("left",x),("top",y),("width",w),("height",h)):
        if key in spec or key in {"x","y","w","h"}: spec[key]=value
...
def _apply_text_evidence(spec:dict,node:dict)->dict|None:
    ...
    lines=applied.get("reference_line_count")
...
def calibrate_reference_deck(deck:dict,layout_path:Path)->tuple[dict,dict|None]:
    ...
    if not route_path.is_file(): return deck,None
    route=_load(route_path)
    if route.get("route")!="reference-reconstruction": return deck,None
    page_graph_path=parent/"page-graph.json"
    if not page_graph_path.is_file(): raise ValueError(f"reference reconstruction requires PageGraph geometry: {page_graph_path}")
