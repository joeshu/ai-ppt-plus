...
def main() -> int:
    ...
    if canvas_evidence_enabled:
        canvas_args = [str(SCRIPT_DIR / "validate_canvas_evidence.py"), "--render-dir", str(render_dir), "--expected-pages", str(args.expected_pages), "--report", str(run_dir / "canvas-evidence.json")]
        if args.reference:
    ...
    if host_validation_enabled:
        host_args = [str(SCRIPT_DIR / "validate_host_validation.py"), str(host_validation_path), "--deck", str(deck), "--expected-pages", str(args.expected_pages), "--report", str(run_dir / "host-validation.json")]
    ...
    for candidate in ("route", "visual-generation", "visual-comparison", "dual-comparison", "canvas-evidence", "host-validation", "source-coverage", "ocr-text-check", "multipage-layout-guard", "preview-consistency", "typography-calibration", "chart-manifest"):
