...
def summarize_report(name: str, path: Path, report: dict):
    ...
    if name == "render_visual_gate":
        ...
        summary.update({"record_count": report.get("record_count"), "repair_required_count": report.get("repair_required_count"), "placement_only_count": report.get("placement_only_count"), "clipping_count": report.get("clipping_count"), "render_crop_evidence_count": report.get("render_crop_evidence_count"), "asset_qa_bound_count": report.get("asset_qa_bound_count"), "repair_loop_required": report.get("repair_loop_required", False)})
    elif name == "protected_repair_plan":
        summary.update({"decision": report.get("decision"), "action_count": report.get("action_count"), "owner_ids": report.get("owner_ids", []), "protection_set": report.get("protection_set", []), "repair_loop_required": report.get("repair_loop_required", False), "human_override": report.get("human_override", False)})
    return summary
...
def main() -> int:
    ...
    add_step("ppt-practical-lint", [str(SCRIPT_DIR / "ppt_practical_lint.py"), str(deck), "--report", str(run_dir / "ppt-practical-lint.json")], outputs=[run_dir / "ppt-practical-lint.json"], inputs=[deck])
    render_args = [str(SCRIPT_DIR / "render_authoritative.py"), str(deck), "--output-dir", str(render_dir), "--dpi", str(args.dpi), "--report", str(render_report_path)]
    if args.font_dir:
    ...
    for candidate in ("route", "visual-generation", "visual-comparison", "dual-comparison", "ocr-text-check", "multipage-layout-guard", "preview-consistency", "typography-calibration", "chart-manifest", "asset-render-coordinate", "protected-repair-plan"):
