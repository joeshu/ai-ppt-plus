...
def alpha_centroid_fit(path: Path, x: int, y: int, width: int, height: int) -> tuple[int, int, int, int]:
    ...
    The returned rectangle is the full transparent canvas placement.  The
    non-transparent subject is aspect-fitted to the requested target and its
    alpha-weighted centroid is aligned to the target center.
    """
    try:
        import numpy as np
    ...
    scale = min(width / visible_w, height / visible_h)
    total = alpha.sum()
    grid_y, grid_x = np.indices(alpha.shape)
    centroid_x = float((grid_x * alpha).sum() / total)
    centroid_y = float((grid_y * alpha).sum() / total)
    canvas_w = max(1, int(round(rgba.width * scale)))
    canvas_h = max(1, int(round(rgba.height * scale)))
    placed_x = int(round(x + width / 2 - centroid_x * scale))
    placed_y = int(round(y + height / 2 - centroid_y * scale))
