...
def plan(data):
    source=data.get("source",{}); jobs=[]; native=[]; profile=str(data.get("execution_profile") or "fast").lower(); retry_limit={"fast":1,"strict":2,"ci":0}.get(profile,1)
    ...
    return {"schema":"ai-ppt-plus/imagegen-asset-jobs/v4","execution_profile":profile,"source":source,"native_editable_assets":native,"jobs":jobs,"job_count":len(jobs),"cache_policy":"batch uncached jobs; retry only failed asset ids; reuse only when cache key, delivered hash, identity/color contract, alpha geometry and local-crop QA evidence all pass","failure_policy":"placement-only repair does not consume regeneration; identity/color or alpha failure retries only failing asset; never silently source-reuse"}
def main():
