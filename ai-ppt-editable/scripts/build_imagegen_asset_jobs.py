...
def _contracts(item):
    ident=item.get("identity_contract") or {}; color=item.get("color_contract") or {}
    identity={"semantic_identity":ident.get("semantic_identity",item.get("semantic_identity")),"contour_traits":ident.get("contour_traits",item.get("contour_traits")),"forbidden_substitutions":list(ident.get("forbidden_substitutions") or [])}
    colors={"foreground":color.get("foreground"),"internal_detail":color.get("internal_detail"),"host_background":color.get("host_background"),"evidence_status":color.get("evidence_status","specified" if color else "unavailable")}
...
def plan(data):
    ...
    for i,item in enumerate(data.get("assets",[]),1):
        ...
        bbox=item.get("source_bbox") or item.get("placement_bbox"); cls=str(item.get("asset_class",item.get("type",""))).lower().replace("-","_"); ref_visible=item.get("reference_visible_bbox_norm"); identity,color=_contracts(item)
        ref={"source_sha256":source.get("sha256"),"asset_id":aid,"asset_class":item.get("asset_class"),"source_bbox":bbox,"aspect_ratio":item.get("aspect_ratio"),"prompt":item.get("prompt"),"alpha_required":item.get("alpha_required",True),"identity_contract":identity,"color_contract":color}
        if ref_visible is not None: ref["reference_visible_bbox_norm"]=list(ref_visible)
        key=stable_hash(ref); alpha_required=bool(ref["alpha_required"]); placement_mode="adaptive-alpha-fit" if alpha_required and cls in ICON_CLASSES else ("alpha-centroid-fit" if alpha_required else "slot-bbox")
        request={"object_id":aid,"generation_prompt":item.get("prompt"),"background_mode":"transparent" if alpha_required else "opaque","preserve_geometry":{"x":bbox[0],"y":bbox[1],"w":bbox[2],"h":bbox[3]} if isinstance(bbox,(list,tuple)) and len(bbox)==4 else {},"align_visible_alpha":alpha_required,"placement_mode":placement_mode,"cache_key":key,"retry_scope":"asset_only","identity_contract":identity,"color_contract":color}
        if ref_visible is not None: request["reference_visible_bbox_norm"]=list(ref_visible)
        qa={"independent_asset":True,"contact_sheet_forbidden":True,"transparent_rgba":alpha_required,"visible_alpha_bbox":True,"alpha_centroid":True,"placement_bbox":True,"local_crop_compare":True,"local_crop_bbox":_crop(bbox),"identity_compare":True,"color_role_compare":color.get("evidence_status")!="unavailable","no_posthoc_tint_substitution":True}
        if ref_visible is not None: qa["placement_geometry_compare"]=True
        jobs.append({"job_id":f"imagegen-{aid}-{key[:12]}","asset_id":aid,"route":"imagegen_asset","cache_key":key,"reference":ref,"output":f"assets/imagegen/{aid}.png","prompt_file":f"assets/prompts/{aid}.txt","request":request,"cache":{"evidence":f"assets/cache/{aid}-{key}.json","reuse_requires":["cache_key_match","asset_sha256_match","alpha_geometry_pass","identity_contract_pass","color_contract_pass_or_unavailable","local_crop_qa_pass"]},"handoff":{"validator":"reconstruction.asset_orchestrator.validate_generated_asset","binder":"reconstruction.asset_orchestrator.bind_generated_asset","placement":"scripts.asset_placement.adaptive_alpha_fit" if placement_mode=="adaptive-alpha-fit" else ("scripts.asset_placement.alpha_centroid_fit" if alpha_required else "slot-bbox")},"qa":qa,"retry_scope":"asset_only","retry":{"policy":"reconstruction.asset_retry_policy.next_retry_request","max_native_attempts":3,"scope":"asset_only","transparent_edit_before_chroma":True,"automatic_source_reuse":False},"state_order":["cache_lookup","native_imagegen","validate_bytes","measure_alpha_geometry","validate_identity_and_color_roles","bind_alpha_centroid","render_local_crop","asset_visual_qa","cache_commit_or_asset_retry"],"source_reuse":"forbidden_without_user_approved_fallback"})
    return {"schema":"ai-ppt-plus/imagegen-asset-jobs/v3","source":source,"native_editable_assets":native,"jobs":jobs,"job_count":len(jobs),"cache_policy":"reuse only when cache key, delivered hash, alpha geometry, identity/color contracts and local-crop QA evidence pass","failure_policy":"retry only failing asset; never silently source-reuse"}
def main():
