...
def _partition_shapes(slide_spec: dict) -> tuple[list[dict], list[dict]]:
    ...
    previous behavior byte-for-byte at the logical layer level.
    """
...
def build_pptx(deck: dict, out_path: Path) -> None:
    ...
    try:
        for slide_no, slide_spec in enumerate(deck["slides"], 1):
            ...
            # independently editable. This is required for progress bars,
            # status badges and cell-level visual markers.
