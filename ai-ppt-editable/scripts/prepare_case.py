...
def prepare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.candidate:
        ...
        candidate_refs.append({"candidate_id": f"candidate-{sha256(destination)[:12]}", "deck": ref(destination, role="candidate-deck", root=output, source_name=candidate.name), "status": "awaiting-evidence", "training_eligible": False})
    else:
    ...
    intake = {
        ...
        "next_action": "run image-to-editable-pptx reconstruction, then attach candidate and QA evidence" if not args.candidate else "run QA gates and obtain explicit human approval",
    }
