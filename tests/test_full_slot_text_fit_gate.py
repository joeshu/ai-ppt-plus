...
def main() -> int:
    ...
    e3_call = compose.index("e3_report = _run_text_fit_e3")
    ...
    # E4 is deliberately a hash-bound receipt, not a fake visual pass.
    assert "pptx_sha256" in compose
    assert "must be paired with render/typography/visual gates" in compose
